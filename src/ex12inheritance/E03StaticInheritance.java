 ...
 */
class Adder {
	/* static으로 선언된 멤버변수는 실제로는 클래스외부(Method영역)에 생성되어 있으므로 클래스명을
	통한 접근이 허용된다. 코드 레벨에서 보면 해당 클래스의 멤버변수이므로 변수명 만으로 접근하는 것도
	가능하다. 즉, 문법적으로는 둘 다 허용된다. */
...
/* 부모의 멤버변수가 public 이므로 접근의 제한이 없다. 따라서 지식 클래스에 작성된 모든 코드는 
문제없이 실행된다. */
