...
class Computer {
	...
	/* 개발자가 생성자를 정의하면 디폴트 생성자는 자동으로 추가되지 않는다. 즉 생성자는
	아래와 같이 필요한 상황에 따라 정의하면 된다. */
	//public Computer(){}
	...
	가능하므로, public으로 선언된 메서드를 통해 간접적으로 호출해야 한다. */
...
class NotebookComputer extends Computer {
	...
	public NotebookComputer(String name, int initCharge) {
		...
		앞에 다른 문장이 먼저 기술되면 에러가 발생한다. 또한 여기서는 부모클래스에
		디폴트 생성자가 없는 상태이므로 생략하는 즉시 에러가 발생한다. */
		super(name);
		//멤버변수 초기화
		battary = initCharge;
	}
	...
	public void movingCal() {
		...
		접근할 수 없어 에러가 발생한다. 이 경우 public으로 선언된 메서드를 통해 호출해야 한다. */
//		keyboardTouch();
//		calculate();
		calculateFunc();
...
class TabletNotebook extends NotebookComputer {
	...
	//태블릿 사용을 위한 배터리랑 체크 및 등록된 펜인지 확인
	public void write(String penInfo) {
		...
		A.compareTo(B)
		: 문자열 A와 B를 비교하여 0이 반환되면 동일한 문자열, -1 혹은 1이 반환되면
		...
		/* 부모클래스에 정의된 메서드를 호출한다. 상속받은 자식클래스에서는 이와 같이 부모의
		멤버메서드를 호출하여 자신의 멤버처럼 사용할 수 있다. 단 접근이 가능한 접근지정자로
		선언되어야 한다. super는 생략할 수 있다.*/
		super.movingCal();
...
public class E04ISAInheritance {
	...
	public static void main(String[] args) {
		...
		noteBook.movingCal(); //=>배터리가 모두 소진되어 사용불가
		noteBook.movingCal();
		noteBook.movingCal();
