...
public class DeChild extends DeParent {
	...
	public DeChild(String name, int age, String stNumber) {
		/* 자식은 부모의 생성자 호출을 통해 먼저 부모인스턴스를 생성한 후 자신의 
		인스턴스를 생성해야 한다. */
		super(name, age);
...
//		System.out.println(name() + "이(가) 공부한다.");
	...
	오버라이딩 아님 : 부모클래스에서 private 멤버로 선언하였으므로 자식쪽에서는 보이지 않아 
		오버라이딩의 대상이 될 수 없다. 해당 메서드는 자식 쪽에서 새롭게 확장한 메서드가 된다.
	 */
	private void eat() {
	...
	-부모에서 default로 선언하면 자식에서는 default 혹은 그보다 넓은 public으로 선언하면
	오버라이딩이 가능하다.
	 */
	@Override
	public String sleep() {
	...
	/*
	메서드의 형태가 부모 쪽과는 다르므로 오버라이딩이 될 수 없다. 이 경우는 오버로딩으로 자식 쪽에서
	확장한 메서드가 된다.
	 */
	...
	/* 
	static(정적) 메서드는 오버라이딩의 대상이 될 수 없다. 클래스 외부(메서드영역)에 미리 로드되므로
	 	오버라이딩 되지 않고, 호출시 각 클래스명을 통해 사용해야 한다. 
	 */
