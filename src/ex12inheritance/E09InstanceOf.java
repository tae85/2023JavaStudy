...
가능하면 true를 반환한다.
-인스턴스간에 형변환이 되려면 반드시 두 클래스간에 상속관계가 있어야 한다.
-부모타입의 참조변수로 자식 인스턴스를 참조할 수 있으므로 매개변수를 부모타입으로 만들면
...
public class E09InstanceOf {
	...
	-Box타입으로 인수를 받게되면 모두 자동형변환(업캐스팅) 되므로 자식 클래스의 멤버메서드는
	호출할 수 없게 된다.
	...
	static void wrapBox(Box b) {
		/*
		세가지 인스턴스 중 어떤 것이 전달되더라도 Box타입으로 자동형변환 되므로 해당 매개변수를
		통해 호출할 수 있는 메서드는 boxWrap()이 유일하다. 따라서 다운캐스팅이 필요하다.
		 */
	...
	/*
	만약 부모타입의 매개변수로 자식인스턴스를 받을 수 없다면 아래와 같이 각각의 타입에
	따른 모든 메서드를 정의해야 한다
