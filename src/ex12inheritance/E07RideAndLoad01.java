...
public class E07RideAndLoad01 {
	...
	public static void main(String[] args) {
		...
		/* 기본자료형의 경우에는 작은타입이 큰타입에 할당되는 것은 자동형변환이 되기 때문에 아래와
		같이 작성할 수 있다. 클래스도 이와 마찬가지로 부모타입(큰타입)으로 자식인스턴스(작은타입)를
		참조할 수 있다. */
		int a = 10;
		double b = 3.14;
		double c = 20;
...
////////////////////////////////////////////////////////////////////////////////////
		/* 부모의 참조변수로 자식의 인스턴스를 참조할 수 있다. 좌측은 부모, 우측은 자식이므로
		모두 가능한 문장이다. */
		A refNum1 = new B();
		A refNum2 = new C();
		B refNum3 = new C();
...
//		C refId3 = refId1;
		...
		/* 자식으로 부모인스턴스를 참조하는 것은 기본적으로 불가능하다. 하지만 다운캐스팅(강제형변환)을
		하면 참조가 가능해진다. 즉 부모타입을 자식타입으로 변경하는 것이다. 이는 클래스가 상속 관계에
		있기 때문에 가능하고, 방식은 기본자료형과 동일하게 소괄호를 이용하면 된다. */
		B refId2 = (B)refId1;
		C refId3 = (C)refId1;
