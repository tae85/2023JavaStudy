...
public class Korea {
	...
	public void publicMethod(String loc) {
		/* public으로 선언된 멤버메서드는 어디서든 접근이 가능하다. 동일패키지, 다른패키지,
		외부클래스, 상속 등 모든 영역에서 접근할 수 있다. */
	...
	protected void protectedMethod(String loc) {
		/* 서로 패키지가 다른 2개의 클래스가 상속관계를 가지고 있다면 접근을 허용한다. 즉 default
		접근 지정자보다 더 넓은 접근범위를 가지고 있다. */
		System.out.println(loc + "protectedMethod() 호출");
		privateMethod("protectedMethod(간접)->");
	}
	
	void defaultMethod(String loc) {
		/* 패키지가 동일할 때만 접근을 허용한다. 패키지가 다른 경우 상속관계가 아니라면 public으로
		선언해야만 접근할 수 있다. */
