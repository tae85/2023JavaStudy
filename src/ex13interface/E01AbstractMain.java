...
/* 
추상메서드를 포함하지 않은 추상클래스를 상속하면 오버라이딩은 강제사항이 아닌 선택사항이다. 즉 필요한
경우에만 오버라이딩 하면 된다. 아래 오버라이딩 한 메서드는 사제해도 에러가 발생하지 않는다.
 */
...
/*
추상클래스2 : 추상메서드를 포함한 클래스, 추상메서드를 멤버로 포함한 클래스는 반드시 abstract로
...
/*
위 클래스를 상속하면 부모가 가진 추상메서드를 그대로 상속하게 되므로 해당 클래스도 abstract로
선언해야 한다. 단, 이 경우 해당 클래스로도 인스턴스를 생성할 수 없게 된다. 따라서 인스턴스를 생성하기
위해서는 부모의 추상메서드를 자식 쪽에서 오버라이딩해야 한다.
 */
class AbsClassChild2 extends AbsClass2 {
	/*
	오버라이딩은 부모 쪽의 메서드를 재정의하는 것이므로 해당 클래스는 추상메서드를 포함하지 않은 상태가
	된다. 따라서 인스턴스 생성이 가능해진다. 아래 오버라이딩한 메서드를 삭제하면 에러가 발생한다.
	 */
	@Override
...
//		AbsClass1 absClass1 = new AbsClass1();
		...
		// 부모 쪽의 멤버메서드는 호출할 수 있다.
		absClass2.absMethod(100);
		// 하지만 자식 쪽은 접근할 수 없으므로 다운캐스팅 해야 한다.
		((AbsClassChild2)absClass2).newMethod();
		
		/* 추상클래스내에 선언된 정적메서드는 오버라이딩의 대상이 될 수 없으므로 static의 기본규칙을
		따른다. 별도의 생성절차 없이 클래스명으로 바로 접근할 수 있다. */
