...
-메서드는 내부에 또 다른 메서드를 정의할 수 없다. 이런 경우 에러가 발생한다.
...
메서드형태1] 매개변수도 없고, 반환값도 없는 메서드
	: 둘 다 없는 형태의 메서드로 주로 메뉴를 출력하거나 하는 단순 출력 기능 정도로
	사용한다.
 */
public class E02MethodType01 {
	...
	static void returnError() {
		...
		실행 되지 않는 쓰레기 코드가 된다. 따라서 return은 반드시 조건문과 함께 
		기술되어야 한다. 단 메서드의 마지막 라인에 기술한다면 조건문이 없어도 상관없다.
		 */
//		return;
		
		//이와 같이 return문이 중간에 있으려면 조건부로 기술해야한다.
		if(returnValue%2 == 0) {
