...
/* 접근지정자 없으면 default 클래스로 정의된다. 이 경우 동일
패키지 내에서만 접근할 수 있다. */
class DefaultClass1 {
	void myFunc() {
...
또한 public 클래스는 자바 파일 하나당 딱 한개만 정의할 수 있다. public으로
정의하는 경우 다른 패키지에서도 접근을 허용하게 되므로 하나의 Java파일에 하나의 
class만 정의하는 것을 권장한다. */
public class E01AccessModifier1 {
	...
	/* protected는 상속관계에 있을 때 접근을 허용하는 지정자이므로 차후 상속
	챕터에서 학습한다. */
	...
	void defaultMethod() {
		privateVar = 200;
		System.out.println("defualtMethod() 메소드 호출");
	}
