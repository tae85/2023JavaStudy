...
-main() 메서드가 실행되기 전에 초기화되므로 main() 메서드 별도의 인스턴스
선언없이 변수를 사용할 수 있다.
...
class MyStatic {
	...
	/* 정적멤버변수 : 인스턴스를 생성할 필요없이 클래스명으로 바로 접근할 수 있다.
	 	JVM(자바가상머신)이 프로그램을 시작할 때 이미 메모리(Method영역)에 로드된다.*/
	...
	/* 정적 멤버메서드 : 정적 메서드 내부에서는 인스턴스형 멤버에는 접근할 수 없다. 정적 멤버만 접근할 
		수 있다. (※ main 메서드에서는 static으로 선언된 메서드만 호출할 수 있었음을 기억하자. */
...
public class E01StaticModifier {
	...
	public static void main(String[] args) {
		...
		접근하든 동일한 메모리가 된다. 따라서 값을 공유하게 되므로 동일한 결과를 출력하게 된다. */
		ms1.staticVar = 800;
		ms2.staticVar = 900;
		...
		/*
		결과를 보듯 입력시에는 각각의 참조변수를 사용했지만 결국 하나의 정적변수에 값을 입력한
		결과가 되므로, 정적변수에 접근시에는 참조변수를 사용하지 말고, 클래스명을 사용해야 한다.
...
//		MyStatic.instanceMethod();
		MyStatic.staticMethod();
		/*
		결론 : 일반적인 인스턴스형 멤버는 인스턴스 참조변수를 통해 접근한다. 정적(static) 멤버는
			클래스명을 통해 인스턴스 생성없이 접근한다.
		 */
	}
