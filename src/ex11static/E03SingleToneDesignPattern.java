...
이를 프로그램 전체에서 공유해서 사용하고자 할 때 쓰는 패턴이다.
...
5.이 방법으로 인스턴스를 생성하면 메모리에 딱 하나만 만들어지게 되므로 메모리를 절약할 수 있다.
 */
//일반적인 클래스 정의
class NoSingleTone {
	int instVar;
...
class SingleTone {
	...
	시작될 때 미리 Method영역에 로드되어 사용할 준비를 마치게 된다. */
...
public class E03SingleToneDesignPattern {
	...
	public static void main(String[] args) {
		/* 일반적인 방식의 인스턴스 생성이므로, new로 생성할 때마다 새로운 참조값(주소값)을 
		할당받게 된다.
		 */
...
//		SingleTone st1 = new SingleTone();
		...
		/* 정적메서드이므로 클래스명을 통해 직접 호출할 수 있다. 해당 메서드 호출을 통해
