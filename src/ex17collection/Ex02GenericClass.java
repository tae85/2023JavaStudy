...
=>기본 Object 기반의 fruitBox를 아래와 같이 제네릭 클래스로 변경한다. 자료형에 해당하는
부분을 타입매개변수로 교체하고 인스턴스 생성시 자료형을 결정하기 위해 클래스명 <T> 형태로 변경한다.
*/
...
public class Ex02GenericClass {
	...
	public static void main(String[] args) {
		/*
		Box 인스턴스를 생성할 때 T부분을 결정하므로 구현의 편의성이 보장된다. 각각 Apple, Banana 
		인스턴스를 저장할 수 있는 상자 인스턴스가 생성되었다.
		 */
		...
		/* Orange 클래스를 저장할 수 있는 상자 인스턴스 생성 해당 클래스는 같은 패키지에 있으므로
		사용 가능하다. */
		GenericFruitBox<Orange> orangeBox = new GenericFruitBox<Orange>();
		/*
		OrangeBox는 인스턴스 생성시 Orange를 저장할 수 있는 용도로 생성되었기 때문에 다른 객체는
		아예 저장할 수 없다. 아래 코드에서 컴파일 에러가 발생하므로 자료형에도 안전한 코드가 된다.
		 */
