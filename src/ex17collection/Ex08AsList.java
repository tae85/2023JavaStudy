...
public class Ex08AsList {
	...
	public static void listPrint(String title, List<String> list) {
		...
		/* 확장 for문으로 전체를 출력한다. 출력할 인스턴스가 String이므로 별도의 오버라이딩 없이도
		내용을 출력할 수 있다. 만약 우리가 직접 정의한 클래스라면 toString()을 오버라이딩 해야 한다.
		 */
		for(Object ob : list) {
			System.out.print(ob + " ");
		}
	...
	public static void main(String[] args) {
		/*
		String 인스턴스 배열을 생성과 동시에 초기화한다. 이 경우 초기데이터를 통해 크기를 유추할 수 
		있으므로 크기는 생략해야 한다.
		 */
		String[] strArr = new String[] {"마린", "파이어뱃", "메딕", "마린"};
		
		/*
		Arrays.asList() : 일반적인 배열을 List<E> 컬렉션으로 변경한 후 반환한다. 단, 값을
			참조만 할 수 있고 입력이나 삭제는 할 수 없다.
		 */
...
//		list.add("탱크");
		...
		/*
		ArrayList<E> 의 생성자를 통해 앞에서 만든 인스턴스를 복사하면 데이터를 변경할 수 있다.
		 */
