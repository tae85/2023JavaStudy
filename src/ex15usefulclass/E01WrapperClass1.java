...
/*
Wrapper 클래스
: 기본자료형의 데이터를 인스턴스화할 때 사용하는 클래스로 Boxing 혹은 Unboxing을 통해 기본자료형의
데이터를 인스턴스화한다. 뒤에서 학습할 제네릭 클래스에서 필수적으로 사용된다.
 */
public class E01WrapperClass1 {
	...
	public static void main(String[] args) {
		...
		/* 
		JDK1.4 이전의 코딩형태
		기본자료형을 박싱 처리한 인스턴스에 대해서 산술연산을 진행할 때 언박싱을 위해 intValue()와
		같은 메소드를 호출해야 한다. 
		*/
		// 현재는 이와 같은 생성자는 Deprecated(디플리케이트) 되었다.
		Integer n10Obj = new Integer(10);
		// 구 번전과의 호환성을 위해 아직 남겨져 있는 상태이다.
		...
		System.out.println("결과값을 2진수로 변경:" + Integer.toBinaryString(result2));
		
		/*
		JDK1.5 이후 버전에서의 코딩형태
		오토박싱 혹은 언박싱이 지원되므로 기본자료형 변수를 선언하는 것과 동일하게 코딩할 수 있다.		
		 */
		Integer numObj1 = 100;
		Integer numObj2 = 200;
		int num3 = numObj1;
