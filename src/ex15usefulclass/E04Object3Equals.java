...
class MyClass {
	...
	모든 인스턴스를 대상으로 하므로 Object타입으로 정의되어 있다.
	 */
	@Override
	public boolean equals(Object obj) {
		/*
		매개변수로 전달된 인스턴스를 Object로 받으면 업캐스팅이 되므로 다운캐스팅 후 비교를 해야 한다.
		이때 전달된 인스턴스가 MyClass형이 아니라면 비교의 대상이 될 수 없으므로 상속관계 확인을 위해
		instanceof 연산자를 사용한다.
		 */
		if(obj instanceof MyClass) {
			/*
			매개변수가 MyClass 타입이 맞다면 비교를 위해 다운캐스팅한다. 그렇지 않으면 부모타입으로는
			자식 멤버에 접근할 수 없으므로 멤버변수 data에 대한 비교가 불가능하다.
			 */
...
public class E04Object3Equals {
	...
	public static void main(String[] args) {
		...
		/*
		인스턴스의 내용을 비교하므로 같다고 출력된다. 만약 오버라이딩 해놓은 equals()를 삭제한 후
		비교하면 다르다는 결과가 나오게 된다. 즉 기본클래스일 때만 오버라이딩 없이 비교가 가능하다.
		 */
