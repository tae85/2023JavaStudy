...
public class E05Date {
	...
	/*
	Calendar 클래스를 통해 얻은 인스턴스를 초로 변환한 후 매개변수로 전달된 날짜서식으로 변경한 문자열을
	반환한다.
	 */
	...
	public static void main(String[] args) {
		...
		유틸리티 메서드(getInstance())를 통해 참조값을 얻어온다.
		 */
		Calendar calendar = Calendar.getInstance(); 
		
		/*
		월을 표현하는 Month, 분을 표현하는 Minute의 이니셜이 동일하므로 구분을 위해 월은 대문자,
		분은 소문자를 사용한다.
		 */
		...
		/*
		현재 날짜를 기준으로 전/후를 구하기 위해 add()를 사용한다. 음수를 인수로 주면 과거의 날짜를 구할 수 있다.
		 */
		...
		/*
		올해 크리스마스까지 남은 날짜를 계산한다. 여기서는 두 가지 서식의 날짜를 파싱하여 구한 날짜 객체를 통해 계산한다.
		 */
		// 오늘날짜 : 0000-00-00 형식
	...
	public static long myDiffDate(String today, String future) {
		...
		try {
			/*
			매개변수를 통해 전달된 2개의 문자열은 날짜의 서식이 서로 다르다. 따라서 각 날짜의 서식을 명확히
			지정해야만 날짜를 인식하여 연산을 할 수 있다. 아래에서는 String을 Date로 변환하고 있다.
			 */
			...
			/*
			getTime()으로 얻어오는 시간은 13자리의 정수값인데 10자리는 순수한 초(Sec)이고, 11~13자리는 
			밀리세컨즈를 의미한다. 즉 100으로 나눠서 뒤의 3자리를 제거하면 순수한 초가 반환된다. 또한 하루는 
